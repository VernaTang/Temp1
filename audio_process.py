...
if __name__ == "__main__":
    ...
    ##
    df = pd.read_csv("./data/original/MELD/test.csv")
    name2sent = {}
    for _, row in df.iterrows():
        name = "dia" + str(row['Dialogue_ID']) + "_utt" + str(row['Utterance_ID'])
        sent = []
        sent.append(row['Sentiment'])
        file_name = name + ".wav"
        ##
        wave_file = os.path.join(current_path, "data/audio_split/MELD/test", file_name)
        print("============================")
        print(file_name + " is handling............")
        if os.path.exists(wave_file):
            rhythm_f = RhythmFeatures(wave_file)
            sent.append(np.average(rhythm_f.intensity()))
            sent.append(np.average(rhythm_f.pitch()))
            sent.append(np.average(rhythm_f.zero_crossing_rate()))

            name2sent[name] = sent

    ##
    audio_output_csv = "./data/test_a_3dim.csv"

    with open(audio_output_csv, 'w') as file:
        file.write("name,Sentiment,intensity,pitch,zcr")
        file.write("\n")
        for row in name2sent.items():
            if row[1][1] != '':
                file.write(re.sub(r'[\[\]\'\s]*', '',str(list(row))))
                file.write("\n")
