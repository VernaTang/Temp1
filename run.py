...
# added the argument "evaluate_during_training" in DynamicTrainingArguments
# Refined the is_master_class() function
import dataclasses
import logging
import os
import sys
...
def main():
    ...
    #本实验中用到的是prompt_demo,因此不走这个if流程
    if data_args.prompt:
        if data_args.prompt_path is not None:
            assert data_args.prompt_id is not None
            prompt_list = []
            with open(data_args.prompt_path, encoding='utf-8', errors='ignore') as f:
                for line in f:
                    line = line.strip()
            ...
            if data_args.template_path is not None:
                with open(data_args.template_path, encoding='utf-8', errors='ignore') as f:
            ...
            if data_args.mapping_path is not None:
                ...
                with open(data_args.mapping_path, encoding='utf-8', errors='ignore') as f:
                    mapping_list = []
                    for line in f:
                        line = line.strip()
        ...
        #此实验用到的是下面这一部分
            ...
            if data_args.template_list is not None:
                for i in range(len(data_args.template_list)):
                    ...
                    #example
                    #| *cls**sent_0*_A*mask*.*sep+* =>
                    # *cls**sent_0*_A*mask*.*sep+**sent_1*_A*label_0*.*sep+**sent_2*_A*label_1*.*sep+**sent_3*_A*label_2*.*sep+*
