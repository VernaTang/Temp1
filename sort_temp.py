...
def main():
    ...
    if len(args.key) == 0:
        if condition['task_name'] == 'cola':
            ...
            print_name = 'SST-2'
        elif condition['task_name'] == 'meld':
            args.key = 'meld_dev_eval_acc'
            args.test_key = 'meld_test_eval_acc'
            print_name = 'MELD'
    ...
    for seed in seed_result:
        ...
        # Load all templates
        with open(os.path.join(args.template_dir, print_name, "{}-{}.txt".format(args.k, seed)), errors='ignore') as f:
            templates = []
            for line in f:
