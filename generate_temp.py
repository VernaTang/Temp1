...
def generate(dataset, template, model, tokenizer, target_number, mapping, beam, label=None, length_limit=None,
             truncate=None):
    """
    ...
    for item in dataset:
        if label is None or item['label'] == label:
            ...
            # print("--------------This is the example of result from get_text()--------------")
            # print(input_text)
    ...
    current_output = [
        {'decoder_input_ids': ori_decoder_input_ids, 'll': 0, 'output_id': 1, 'output': [], 'last_length': -1}]
    for i in tqdm(range(max_length - 2)):
        ...
        for item in current_output:
            ...
            #print("This is aggr_output before", aggr_output.size())
            aggr_output = aggr_output.mean(0)
            #print("This is aggr_output after", aggr_output.size())
...
def search_template(model, tokenizer, task_name, k, seed, beam, output_dir, data_dir):
    ...
    map_of_mapping = {
        ...
        'MELD': {'positive': 'positive', 'negative': 'negative', 'neutral': 'neutral'}
    }
    ...
    if task_name == "MELD":
        ...
        generate_text = generate(dataset, template, model, tokenizer, target_number=2,
                                 mapping=mapping, beam=beam, label=None, truncate='head')[:beam//2]

        print("####### generated templates in head#######")
        ...
        generate_text = generate(dataset, template, model, tokenizer, target_number=2, mapping=mapping, beam=beam,
                                 label=None, truncate='tail')[:beam//2]

        print("####### generated templates in tail #######")
        for text in generate_text:
...
def main():
    ...
    parser.add_argument('--task_name', type=str, nargs='+',
                        default=['SST-2', 'sst-5', 'mr', 'cr', 'subj', 'trec', 'CoLA', 'MRPC', 'QQP', 'STS-B', 'MNLI',
                                 'SNLI', 'QNLI', 'RTE'], help="Task names")
    parser.add_argument('--output_dir', type=str, default='my_auto_template')
    ...
    for task_name in args.task_name:
        for seed in args.seed:
            search_template(model=model, tokenizer=tokenizer, task_name=task_name, k=args.k, seed=seed, beam=args.beam,
                            output_dir=args.output_dir, data_dir=args.data_dir)


if __name__ == '__main__':
    # dataset = load_dataset("MELD","data/k-shot/MELD/16-13")
    # print(dataset[0])
    # template = "*cls**sentu_0**<extra_id_0>**label**<extra_id_1>**sep+*"
    ...
    # model = model.cuda()
    #
    # generate(dataset, template, model, tokenizer, target_number=2,
    #          mapping=mapping, beam=100, label=None, truncate='head')
    main()